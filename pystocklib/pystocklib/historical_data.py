...
class HistoricalData:
	...
	def create_csv(self):
		if self.path is not "":
			self.df.to_csv('{}.csv'.format(self.path +"/"+ self.stock_name), encoding='utf-8')
		else:
	...
	def moving_average(self, values):
		...
		ma = self.df[values].rolling(window=20, center=False).mean()
		return ma
	...
	def info_plot(self, col, col2):
		self.price_avg_hl(col)
		self.open_close(col, col2)

	def price_avg_hl(self, col):
	...
	def open_close(self, col, col2):
		subplot_info2(self.stock_name, [
			[self.df[col], 'Close Price'],
			[self.df[col2], 'Open Price']
		], path=self.path + '/')
